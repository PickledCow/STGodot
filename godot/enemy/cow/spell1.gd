		...
		if u >= CYCLE_RATE[difficulty]:
			if u % RATE2[difficulty] == 0:
				...
				for i in DENSITY[difficulty]:
					Bullets.create_bullet(position, SPEED2[difficulty], (i+1) * 360.0/DENSITY[difficulty] + 90, 0.0, 0.0, Constants.BULLET_TYPE.RICE, Constants.COLOURS.TEAL_D, false)
	t += 1
